...
const MOCK_REPUTATION: ReputationData = {
  address:            "0xAbCd...1234",
  score:              4.2,
  total_contracts:    18,
  completed:          16,
  ...
  signal_tags:        ["Fast delivery", "Clear communication", "Reliable", "1 ghost incident"],
  ai_summary:         null,
};
...
function tagStyle(tag: string): string {
  ...
  if (lower.includes("ghost") || lower.includes("dispute") || lower.includes("lost"))
    return "border-red-500/30 bg-red-500/10 text-red-400";
  if (lower.includes("fast") || lower.includes("reliable") || lower.includes("clear"))
...
function tagIcon(tag: string): string {
  ...
  if (lower.includes("clear") || lower.includes("communication")) return "bi-chat-dots";
...
function ScoreBar({ score }: { score: number }) {
  ...
  const label   = score >= 4.5 ? "Excellent" : score >= 3.5 ? "Good" : score >= 2.5 ? "Fair" : "Poor";
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className={`shrink-0 rounded-full border px-2 py-0.5 text-xs font-medium ${
        score >= 4 ? "border-green-500/30 bg-green-500/10 text-green-400"
        : score >= 3 ? "border-yellow-500/30 bg-yellow-500/10 text-yellow-400"
...
function AISummaryBox({ address }: { address: string }) {
  ...
  const fetchSummary = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/ai/reputation-summary`, {
        method: "POST",
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/5 p-3">
      ...
      >
        ...
        {loading && <div className="ml-auto h-3 w-3 animate-spin rounded-full border border-green-400 border-t-transparent" />}
        {!loading && <i className={`bi ${expanded ? "bi-chevron-up" : "bi-chevron-down"} ml-auto`} />}
      </button>
      {expanded && summary && (
...
export function ReputationCard({ address }: { address: string }) {
  ...
  useEffect(() => {
    async function fetchRep() {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/users/${address}/reputation`);
        if (!res.ok) throw new Error();
        setRep(await res.json());
      } catch {
        setRep(MOCK_REPUTATION);
      } finally {
        setLoading(false);
      }
    }
  ...
  const completionRate = rep.total_contracts > 0
    ? Math.round((rep.completed / rep.total_contracts) * 100)
    : 0;
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-5 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-gray-500">Party A Reputation</p>
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          { icon: "bi-file-earmark-check", label: "Contracts",  value: rep.total_contracts, color: "text-white"        },
          { icon: "bi-check-circle",       label: "Completed",  value: `${completionRate}%`, color: "text-green-400"   },
          { icon: "bi-trophy",             label: "Won",        value: rep.disputes_won,    color: "text-blue-400"     },
          { icon: "bi-person-dash",        label: "Ghosts",     value: rep.ghosting_incidents, color: rep.ghosting_incidents > 0 ? "text-red-400" : "text-gray-500" },
        ].map(({ icon, label, value, color }) => (
          <div key={label} className="flex flex-col items-center rounded-xl border border-white/10 bg-white/5 p-2 text-center">
